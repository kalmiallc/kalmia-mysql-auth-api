...
def notifyBuild(String buildStatus = 'SUCESSFULL', String additionalMessage = '') {
  ...
  // Send notifications
  slackSend (channel: '#ci-monitor', color: colorCode, message: summary)
}

pipeline {
     parameters {
        choice(name: 'LOG_OUT_LEVEL', choices: ['INF', 'DBG', 'VBS'], description: 'Log level')
        string(name: 'MYSQL_DB_TEST', defaultValue: 'test-lib-db', description: '')
        ...
        string(name: 'MYSQL_DB', defaultValue: 'test-lib-db', description: '')
  ...
  stages {
     stage ('Clone repository') {
         steps {
            ...
            submoduleCfg: [], 
            userRemoteConfigs: [[url: 'git@bitbucket.org:kalmiadevs/kalmia-mysql-auth-api.git',
     ...
     stage('Build Image') {
       steps {
           sh '''
             ...
             echo "Preparing image ${IMAGE}:${RELEASE}-$F"       
             AUTH_REPO_ACCESS_KEY="$(cat kalmia-auth-repo-access.key)" docker-compose -f docker-compose.test.yaml --env-file ./.env.test build --force
           '''
        }
      }
